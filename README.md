...
<details><summary style="color:skyblue">JPA란 무엇인가요?</summary>

[🔭 자세한 설명](https://velog.io/@prettylee620/JPA%EB%9E%80-%EB%AC%B4%EC%97%87%EC%9D%B8%EA%B0%80%EC%9A%94)
...
데이터를 생성한 프로그램의 실행이 종료되더라도 사라지지 않는 데이터의 특성을 의미하며, 영속성은 파일 시스템, 관계형 데이터베이스 혹은 객체 데이터베이스 등을 활용하여 구현한다.

2. JPA는 ORM(Obeject Relational Mapping) 기술이다.

> 모델링 한다는 것은 추상화적인 개념을 현실세계에 구체화하는 것을 말한다. JPA경우 첫 번째로 자바에서 코드를 작성한 것인 클래스를 만들어서 두 번째로 데이터베이스 테이블을 자동으로 생성 수 있다. 이때 필요한 것이 `인터페이스`이다.

3. JPA는 반복적인 CRUD 작업을 생략하게 해준다.
자바 Object로 변환을 해야 하는데 이것은 단순한 반복로직이다. 이러한 과정들을 connction을 연결하고 변환하고 등등을 JPA가 하나의 함수로 제공해준다.

4. JPA는 영속성 컨텍스트를 가지고 있다.

⭐ **영속성 컨텍스트란?**

자바가 데이터베이스를 저장해야 하는 모든 데이터를 아는 것이 영속성 컨텍스트

자바에 있는 동물 데이터를 영속성 컨텍스트에 넣고 나서 DB에도 저장 후 영속성 컨텍스트에서 삭제를 하면 동기화가 되어 있기에 DB에서도 삭제가 된다.

> 데이터 베이스에 있는 데이터를 select해서 가져오고 update하는 등 일련의 모든 정보를 영속성 컨텍스트를 통해 확인 할 수 있다.

5. JPA는 DB와 OOP의 불일치성을 해결하기 위한 방법론을 제공한다.(DB는 객체저장 불가)

> 즉, JPA는 DB와 OOP의 불일치성을 해결하기 위한 방법론을 제공한다는 것은 먼저, 데이터베이스가 주도권을 잡고 있다면 데이터베이스는 Object를 저장할 수 없기에 OOP와 불일치하게 되지만 JPA를 사용하게 되면 OOP를 만들 수 있게 된다. 

`객체 지향적`으로 만들 수 있다는 것이다. 그리고 ORM을 하게 되면 자바가 주도권을 가진 즉, 위와 같은 형태의 데이터 베이스를 만들 수 있다.
...
<details><summary style="color:skyblue">스프링부트 동작원리</summary>

[🔭 자세한 설명](https://velog.io/@prettylee620/wo6fkf38)

🪄 간단한 설명

**1. 소켓 통신**

- 내장 톰캣 쓰는 소켓 통신은 네트워크 연계 중 하나야
- 서버는 소켓 만들고 포트 할당하고 클라이언트가 오면 응답
- `포트번호`로 특정 사용자랑 연결돼, 스레드로 여러 사용자 동시에 통신 가능
- 근데 연결이 계속 유지돼서 부하 증가 가능성 있다.

**2. 웹 서버**

- 웹 서버는 클라이언트 요청 받아서 처리하고 응답하는 역할
- HTTP 프로토콜로 돌아가고, Stateless해서 각 요청 독립적으로 처리
- 정적인 건 서버가, 동적인 건 애플리케이션 서버에 맡길 수 있다.
- Apache, Nginx, Microsoft IIS 같은 거 예시

**3. 웹 서비스**

- 웹 서비스는 다른 시스템끼리 표준 방식으로 통신하기 위한 규격을 제공
- XML이나 JSON으로 데이터 주고받아, RESTful API나 SOAP 이런 게 웹 서비스 예시
- 네트워크로 상호작용하면서 플랫폼 간 통신 가능케 함.

**4. HTTP**

- HTTP는 웹에서 텍스트 문서 교환하는 규약으로, 암호화 없이 정보 전송
- 보안성 떨어지지만, 웹에서 데이터 주고받을 때 주로 사용

</details>

<details><summary style="color:skyblue">톰켓이란?</summary>

[🔭 자세한 설명](https://velog.io/@prettylee620/%ED%86%B0%EC%BC%93%EC%9D%B4%EB%9E%80-%EB%AC%B4%EC%97%87%EC%9D%B8%EA%B0%80)

🪄 간단한 설명

**1. 시스템 콜(System Call)**

- 시스템 콜은 프로그램이 OS 커널이 제공하는 서비스를 이용하고 싶을 때 사용되는 메커니즘
- 프로세스나 스레드가 OS의 기능을 활용하려면 시스템 콜을 통해 커널 모드에서 실행되어야 한다.
- 시스템 콜은 여러 종류가 있으며, 프로세스/스레드, 파일 I/O, 소켓, 장치, 프로세스 통신 등 다양한 영역에서 사용된다.

**2. 시스템 콜 & 인터럽트 예제: 파일 Read**

- 프로세스가 파일을 읽으려면 시스템 콜을 호출하고, 이때 커널 모드로 전환되며 해당 작업이 완료될 때까지 기다린다. 
- 인터럽트는 파일이 읽을 준비가 되었음을 알려주며, 다시 커널 모드로 전환하여 작업을 처리

**3. 프로그래밍 언어와 시스템 콜**

- 프로그래밍 언어는 일반적으로 개발자가 직접 시스템 콜을 사용하지 않고, 언어가 제공하는 라이브러리 등을 통해 간접적으로 사용
- 예를 들어, Java에서의 Thread 클래스는 내부에서 네이티브 인터페이스를 통해 운영체제의 시스템 콜을 호출
- 언어는 특정 운영체제와의 상호작용을 간소화하여 개발자가 편리하게 프로그래밍할 수 있도록 지원

**4. 웹 서버(Apache)와 톰켓(Tomcat)**

- `웹 서버`는 클라이언트의 요청에 대해 정적인 자원(HTML, 이미지 등)을 응답하는 역할을 한다.
- `Apach`e는 대표적인 웹 서버 중 하나다.
- 톰켓은 Java 언어로 작성된 동적인 웹 페이지를 처리하는 서블릿 컨테이너로, 웹 서버와 연동하여 동적인 콘텐츠를 생성한다.
- Apache와 톰켓을 함께 사용하여 정적 및 동적인 웹 콘텐츠를 처리할 수 있다.
...
# HTTP 웹 지식
<details><summary style="color:skyblue"> 인터넷 네트워크</summary>

[🔭 자세한 설명](https://velog.io/@prettylee620/HTTP-%EC%9B%B9-%EC%A7%80%EC%8B%9D-HTTP%EC%9D%98-%EA%B8%B0%EB%B3%B8)

🪄 간단한 설명
- 인터넷 통신은 IP 프로토콜을 통해 이뤄지고, TCP는 연결 지향적이고 신뢰성이 높은 가상 회선 방식, UDP는 간단하고 빠른 데이터그램 방식이야. 
- TCP는 연결 설정 후 안전하게 데이터 전송하고, UDP는 빠르게 전송하되 순서나 에러에 대한 보장은 없어. 포트는 프로세스 식별에 쓰이고, DNS는 도메인 명을 IP 주소로 변환해줘
